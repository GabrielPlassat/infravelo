...
/* ── GROS BOUTONS ── */
/* On cible les vrais boutons Streamlit */
...
.result-verdict {
    font-size: 14px; color: #8B8FA8; margin-top: 2px;
}
.pill {
...
try:
    ...
    st.error("⚠️ Clé API manquante — ajoutez ANTHROPIC_API_KEY dans Streamlit Secrets")
    st.stop()
...
def sauver(r, photo_bytes, lat, lng):
    ...
    <div class="result-card" style="border-left-color:{c}">
        ...
        <div class="criteres-grid">
            <div class="critere-box">
                <div class="critere-val" style="color:{couleur(crit['largeur'])}">{crit['largeur']}</div>
                <div class="critere-name">Largeur</div>
            </div>
            <div class="critere-box">
                <div class="critere-val" style="color:{couleur(crit['obstacles'])}">{crit['obstacles']}</div>
                <div class="critere-name">Obstacles</div>
            </div>
            <div class="critere-box">
                <div class="critere-val" style="color:{couleur(crit['signalisation'])}">{crit['signalisation']}</div>
                <div class="critere-name">Signalisation</div>
            </div>
            <div class="critere-box">
                <div class="critere-val" style="color:{couleur(crit['separation'])}">{crit['separation']}</div>
                <div class="critere-name">Séparation</div>
            </div>
        </div>
...
with onglet_terrain:
    ...
    if lat:
        ...
        st.caption("📍 GPS non disponible — autorisez la localisation")
    ...
    if st.session_state.serie_active and st.session_state.serie_restant > 0:
        ...
        <div class="progress-serie">
            <div style="font-size:15px;font-weight:700;color:#2979FF">
                Photo {n_fait + 1} / 10
            </div>
            <div style="font-size:12px;color:#555A72;margin-top:2px">
                Prochaine dans 5 secondes…
            </div>
        ...
        photo_serie = st.camera_input("", label_visibility="collapsed",
                                       key=f"cam_serie_{n_fait}")
        if photo_serie:
            ...
            if r:
                ...
                if st.session_state.serie_restant > 0:
                    ...
                    st.success(
                        f"✅ Série terminée — Score moyen : "
                        f"{sum(sc)/len(sc):.1f}/5 "
                        f"({min(sc)} → {max(sc)})"
                    )
    ...
    if nb > 0:
        st.markdown(f"""
        <div style="font-size:12px;color:#555A72;
        ...
        if st.button("Effacer la session", use_container_width=True):
            for k in ["historique","derniere","serie_active","serie_restant","serie_scores"]:
                st.session_state[k] = [] if k in ["historique","serie_scores"] else (False if "active" in k else (0 if "restant" in k else None))
            st.rerun()
    ...
    if nb == 0:
        ...
        <div style='text-align:center;padding:60px 20px;color:#555A72'>
            ...
            <div style='margin-top:12px;font-size:15px'>
                La carte apparaît après la première analyse
            </div>
        </div>""", unsafe_allow_html=True)
    else:
...
with onglet_rapport:
    if nb == 0:
        ...
        <div style='text-align:center;padding:60px 20px;color:#555A72'>
            ...
            <div style='margin-top:12px;font-size:15px'>
                Le rapport se remplit au fil des analyses
            </div>
        </div>""", unsafe_allow_html=True)
    else:
        # Résumé chiffré
        ...
        <div style="background:#1A1D27;border-radius:16px;padding:20px;margin-bottom:16px">
            <div style="font-size:13px;color:#555A72;text-transform:uppercase;letter-spacing:1px">
                Score moyen du parcours
            </div>
            <div style="font-size:48px;font-weight:900;color:{couleur(round(moy))};margin:4px 0">
                {moy:.1f}/5
            </div>
            ...
            for i, h in enumerate(st.session_state.historique, 1):
                ...
                <div class="histo-row" style="flex-direction:column;align-items:flex-start;gap:4px">
                    ...
                    <div style="font-size:11px;color:#555A72">
                        Largeur {s['criteres']['largeur']} ·
                        Obstacles {s['criteres']['obstacles']} ·
                        Signalisation {s['criteres']['signalisation']} ·
                        Séparation {s['criteres']['separation']}
                    </div>
